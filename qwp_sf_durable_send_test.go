...
// TestQwpDurableAckSeqGapHalts pins the fail-closed defense against a server that
// coalesces or drops an OK ack in durable mode. Skipping a wire sequence would
// leave the skipped frame with no pending entry, so a later durable ack could
// trim past it — silent loss of data the API promises is durable. The client must
// instead HALT with PROTOCOL_VIOLATION and never advance the watermark.
func TestQwpDurableAckSeqGapHalts(t *testing.T) {
	ctx := context.Background()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set(qwpHeaderVersion, "1")
		w.Header().Set(qwpHeaderDurableAck, qwpDurableAckEnabledValue)
		...
		// Coalesced/dropped ack: OK for wireSeq 2, skipping 1 — the documented hole.
		_ = conn.Write(ctx, websocket.MessageBinary, buildAckOKWithTables(2, ackTableEntry{"trades", 2}))
		for {
			if _, _, err := conn.Read(ctx); err != nil {
				return
			}
	...
	if got := qs.AckedFsn(); got >= 0 {
		t.Fatalf("AckedFsn advanced to %d despite the sequence gap; it must stay frozen", got)
	}
}
