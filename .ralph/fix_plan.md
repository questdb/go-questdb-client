...
- [ ] Create `qwp_buffer.go` with `qwpColumnBuffer` struct: stores column name, typeCode, nullable flag, scale (for decimals), and type-specific data slices (fixedData []byte with fixedSize stride, boolData []byte for bit-packing, strOffsets []uint32 + strData []byte for strings, symbolIDs []int for symbols, nullBitmap []byte, nullCount). Implement methods: addLong(int64), addDouble(float64), addBool(bool), addString(string), addSymbolID(int), addNull(), addTimestamp(int64), addByte(int8), addShort(int16), addInt32(int32), addFloat32(float32), addChar(rune), addUuid(hi,lo uint64), addLong256(bytes), reset(). Write tests verifying backing bytes for each type.
- [ ] Add `qwpTableBuffer` struct to `qwp_buffer.go`: tableName, columns []qwpColumnBuffer, columnIndex map[string]int, rowCount, schemaHash/schemaValid. Methods: getOrCreateColumn(name, typeCode, nullable) → *qwpColumnBuffer (with type conflict detection), commitRow() (gap-fills missing columns with null/sentinel), cancelRow() (rollback to checkpoint), reset(), getSchemaHash(). Write tests for multi-column tables, gap-filling, type conflicts, row cancel.
- [ ] Add array column support to `qwpColumnBuffer`: arrayMeta []qwpArrayMeta (nDims uint8, shape []int32) + arrayData []byte (flattened LE elements). Methods: addDoubleArray(nDims, shape, flatData), addLongArray(nDims, shape, flatData). Integrate with 1D/2D/3D helpers matching existing API. Write tests.
- [ ] Add decimal column support to `qwpColumnBuffer`: store big-endian unscaled values in fixedData (8/16/32 bytes depending on decimal type). Track scale (must be consistent across rows). Reuse existing Decimal type from decimal.go. Write tests.

## Phase 3: QWP Message Encoder

- [ ] Create `qwp_encoder.go` with `qwpEncoder` struct: owns a `qwpWireBuffer` scratch. Method `encodeTable(tb *qwpTableBuffer, schemaMode byte, schemaHash int64) []byte` that writes: header (12 bytes with placeholder payload len) → table name → rowCount → colCount → schema → column data → patches payload length. Start with fixed-width column encoding only (LONG, DOUBLE, TIMESTAMP, BYTE, SHORT, INT, FLOAT, CHAR, DATE, UUID, LONG256, TIMESTAMP_NANOS). Write golden byte test: encode a table with 2 rows of (LONG, DOUBLE), verify output byte-for-byte.
- [ ] Add boolean column encoding to the encoder: bit-packed output. Add null bitmap encoding for nullable columns. Write golden byte tests: 3 bools → expected packed byte, nullable column with 1 null → expected bitmap + data.
- [ ] Add string/varchar column encoding: write (rowCount+1) uint32 LE offsets then string data. Add symbol column encoding for delta dict mode (varint global IDs per row). Write golden byte tests for strings ("hello", "world") and symbols with known global IDs.
- [ ] Add array column encoding: per-row nDims(1B) + shape(4B each) + flattened elements(8B each). Add decimal column encoding: big-endian unscaled values with scale in schema. Write tests.
